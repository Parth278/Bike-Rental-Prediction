...
setwd("C:/Users/Best Lappy/Desktop/EDWISOR/Projects/2")
...
#removing instant,casual and registered
df[ ,c('instant', 'casual', 'registered')] <- list(NULL)
...
for(i in 1:length(num_var)){
  assign(paste0("AB",i),ggplot(aes_string(x="cnt",y=(num_var[i])),d=df)+
           ...
           ggtitle("Boxplot of count of bikes with",cat_var[i]))
}

gridExtra::grid.arrange(AB1,AB2,AB3,ncol=3)
gridExtra::grid.arrange(AB4,ncol=1)
